export function getTimeStamp(askDate){            //gets and formats time
    ...
    if (years > 0){
      return month[date.getMonth()] + " " + date.getDate() + ", " + date.getFullYear() + " at " + date.getHours() + ":" + date.getMinutes().toString().padStart(2, '0') + "."
    } else if(days > 0){
        return month[date.getMonth()]  + " " + date.getDate() + " at " + date.getHours() + ":" + date.getMinutes().toString().padStart(2, '0') +  ".";
    } else if(hours > 0){
  ...
  export function questionErrors(){                     // function for errors in post question page
    let flag = 0;
    ...
    if((titleT.length > 100) || (titleT.length < 1)){
      document.getElementById("title_error").textContent = "Title cannot be empty and cannot be greater than 100 characters!";
      flag = 1;
    }
    if((textT.length < 1)){
    ...
    if((newtagarray.length > 5) || (a === 1)){
      a = 0;
      document.getElementById("tags_error").textContent = "There can't be more than 5 tags and each tag cannot be greater than 20 characters!";
      flag = 1;
    }
  ...
  export function answerErrors() {
    ...
    if((usernameT.length < 1)){
      document.getElementById("ans_username_error").textContent = "Username cannot be empty!";
      flag = 1;
    }
    if((textT.length < 1)){
      document.getElementById("ans_text_error").textContent = "Text cannot be empty!";
      flag = 1;
    }
    return flag;
  }